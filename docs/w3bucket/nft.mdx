...
Each W3Bucket is an [ERC721](https://eips.ethereum.org/EIPS/eip-721) NFT token (with symbol name *W3BKT*) that could be minted and traded on NFT marketplaces like [OpenSea](https://opensea.io).

W3Buckets are fully bound to *W3BKT NFTs*:

- Essential information of W3Buckets are put in *W3BKT NFTs* metadata, like capacity, file history, etc
- Only a *W3BKT NFT* owner could upload files or folders to the bound W3Bucket
- When a *W3BKT NFT* is transferred to another user, the W3Bucket data contents are also transferred
